...
export GRADLE_HOME=/opt/gradle/7.6.3
export TOOL_VERSION=3.9.5
export PROJECT_VERSION=4.7.5
...
if [ -n "" ]; then
  echo "Setting version to 4.7.5 to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="4.7.5" | tee /root/project/logs/enforce-version.log
fi
